...
export class DashboardPage {
  ...
  static render() {
    ...
    // Szybkie podsumowanie: grupujemy zadania po przedmiocie i liczymy statusy
    const widgets = document.createElement("div");
    widgets.classList.add("dashboard-widgets");

    // Wybieramy/łączymy dane z nowych stałych importowanych z constants.js

    console.log("TASKS_STUDENT_DATA_ASO_0:", TASKS_STUDENT_DATA_ASO_0);
    console.log("TASKS_STUDENT_DATA_BD_0:", TASKS_STUDENT_DATA_BD_0);

    const TASKS_DATA = []
      .concat(
        Array.isArray(TASKS_STUDENT_DATA_ASO_0) ? TASKS_STUDENT_DATA_ASO_0 : []
      )
      .concat(
        Array.isArray(TASKS_STUDENT_DATA_BD_0) ? TASKS_STUDENT_DATA_BD_0 : []
      );
    ...
    if (Array.isArray(TASKS_DATA) && TASKS_DATA.length) {
      ...
      widgets.appendChild(widgetsSubjectSummaryWrapper);
    } else {
      const empty = document.createElement("p");
      empty.classList.add("no-data");
      empty.textContent = "Brak danych zadań do wyświetlenia.";
    ...
    widgets.appendChild(this.examSection(EXAM_STUDENT_DATA_0));
