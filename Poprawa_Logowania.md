...
- [ ] **Modyfikacja `Core\Auth\AuthController.php`**:
    - [ ] Dodać metodę `check()` (endpoint `/api/auth/check` lub `/api/me`), która zwraca status sesji (zalogowany/niezalogowany) oraz rolę użytkownika.
- [ ] **Modyfikacja `backend/core/api.php`**:
    - [ ] Dodać logikę "Middleware" przed wywołaniem kontrolerów.
    - [ ] Sprawdzać `SessionManager::isLoggedIn()`.
    - [ ] Jeśli użytkownik nie jest zalogowany, a próbuje dostać się do zasobów chronionych (wszystko poza `login`), zwrócić kod `401 Unauthorized`.
- [ ] **Weryfikacja Ról (RBAC)**:
    - [ ] Dodać sprawdzanie roli w kontrolerach wrażliwych (np. `UserController` dostępny tylko dla Admina/Nauczyciela, `TaskController` (edycja) dla Nauczyciela).
...
- [ ] **Stworzenie `js/Class/AuthService.js`**:
    - [ ] Metoda `login(username, password)` – strzela do API, po sukcesie *nie zapisuje roli w localStorage*, ew. tylko flagę "isLoggedIn" dla UX, ale prawdziwa weryfikacja dzieje się przez sesję (cookie).
    - [ ] Metoda `checkSession()` – wywoływana przy starcie Dashboardu. Pyta backend o status. Jeśli 401 -> przekierowanie na Login.
    - [ ] Metoda `logout()` – strzela do API `/logout` i czyści stan frontendu.

### Krok 3: Refaktoryzacja Dashboardu (`script3.js`)
- [ ] Usunąć logikę pobierania roli z `localStorage` (`const role = localStorage.getItem('user_role')`).
- [ ] Zastąpić to wywołaniem `AuthService.checkSession()` na starcie.
    - [ ] Czekamy na odpowiedź serwera.
    - [ ] Serwer zwraca: `{ loggedIn: true, role: 'teacher', name: '...' }`.
    - [ ] Dopiero wtedy renderujemy menu (`DashboardRender`) z odpowiednimi opcjami.
    - [ ] Loader/Spinner na czas weryfikacji sesji, aby użytkownik nie widział "migającej" treści.

### Krok 4: Czyszczenie `Login.js`
- [ ] `Login.js` powinien korzystać z `AuthService`.
- [ ] Usunąć `localStorage.setItem('user_role', ...)` – to jest zbędne i niebezpieczne jako źródło prawdy.
