    ...
    public function check() {
        if ($this->session->isLoggedIn()) {
            return json_encode([
                ...
                'menu' => $this->getMenuForRole($_SESSION['role'] ?? 'student')
            ]);
        } else {
            http_response_code(401);
            return json_encode(['loggedIn' => false, 'error' => 'Unauthorized']);
        }
    }

    private function getMenuForRole($role) {
        $menuConfig = require __DIR__ . '/../../config/menu.php';
        $filteredMenu = [];

        foreach ($menuConfig as $item) {
            if (in_array($role, $item['roles'])) {
                // Usuwamy pole 'roles' z odpowiedzi dla czystości
                unset($item['roles']);
                $filteredMenu[] = $item;
            }
        }
